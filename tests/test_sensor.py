...
class TestVisitorCounter(unittest.TestCase):
    ...
    def test_weekdays_opened(self):
        """
            ...
            week : monday 11/9/2023 to friday 15/9/2023 (16 excluded in the range)
            HOUR : 9 AM
        :return:
        """
        for test_day in range(11, 16):
            with self.subTest(i=test_day):
                visit_count = self.visitors.get_visit_count(datetime(2023, 9, test_day, 9))
                self.assertFalse(visit_count == -1)

    def test_hours_opened(self):
        """
            Test a worked day for hours in [9:12, 13:17] : not -1 (opened)
            weekday : wednesday 11/12/2024 to friday 15/9/2023 (16 excluded in the range)
            hours : 9 to 17 (without 12)
        :return:
        """
        for test_hour in [9,10,11,13,14,15,16,17]:
            with self.subTest(i=test_hour):
                visit_count = self.visitors.get_visit_count(datetime(2024, 12, 11, test_hour))
                self.assertFalse(visit_count == -1)
    ...
    def test_weekend_closed(self):
        ...
        for test_hour in [0,9,12,15,22]:
            with self.subTest(i=test_hour):
                visit_count = self.visitors.get_visit_count(datetime(2024, 12, 21, test_hour))
                self.assertTrue(visit_count == -1)

        for test_hour in [0,9,12,15,22]:
            with self.subTest(i=test_hour):
                visit_count = self.visitors.get_visit_count(datetime(2024, 11, 10, test_hour))
                self.assertTrue(visit_count == -1)

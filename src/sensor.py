...
class VisitorCounter:
    ...
    def get_visit_count(self, dt:datetime):
        ...
        try:
            ...
            random_visit_count = np.random.normal(self.avg_visitor_count,self.std_pct*self.avg_visitor_count)

            if   dt.weekday() == 0: day_avg_visits = random_visit_count * 1.
            elif dt.weekday() == 1: day_avg_visits = random_visit_count * 1.25
            elif dt.weekday() == 2: day_avg_visits = random_visit_count * 1.3
            elif dt.weekday() == 3: day_avg_visits = random_visit_count * 1.6
            elif dt.weekday() == 4: day_avg_visits = random_visit_count * 1.8
            elif dt.weekday() == 5: day_avg_visits = 0
            elif dt.weekday() == 6: day_avg_visits = 0
            else : raise ValueError("weekday not between 0 and 6!")

            if   dt.hour ==  9: hour_visits = int(day_avg_visits * 0.2)
            elif dt.hour == 10: hour_visits = int(day_avg_visits * 0.25)
            elif dt.hour == 11: hour_visits = int(day_avg_visits * 0.25)
            # elif dt.hour == 12: hour_visits = 0
            elif dt.hour == 13: hour_visits = int(day_avg_visits * 0.1)
            elif dt.hour == 14: hour_visits = int(day_avg_visits * 0.1)
            elif dt.hour == 15: hour_visits = int(day_avg_visits * 0.02)
            elif dt.hour == 16: hour_visits = int(day_avg_visits * 0.03)
            elif dt.hour == 17: hour_visits = int(day_avg_visits * 0.05)
            else : hour_visits = 0
...
if __name__ == "__main__":
    ...
    print(f'This day {date.day}/{date.month}/{date.year}, '

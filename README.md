...
À coller **en remplaçant** le bloc existant “Installation et utilisation”.
...
### 2️⃣ Créer l’environnement avec uv

```bash
# Installer uv si besoin
# pip install uv

# Installer les dépendances depuis pyproject.toml / uv.lock
uv sync
...
Le code applicatif vit dans `src/bank_footfall` (layout « src »).
...
L’extraction moderne est dans `src/bank_footfall/etl/extract.py` et utilise la config centrale (`bank_footfall.config.settings`).
...
L’ancien script CLI (`extract_legacy.py`) est conservé pour référence, mais le pipeline utilise désormais `bank_footfall.etl.extract`.

### 5️⃣ Prérequis Java pour PySpark
...
```text
JAVA_HOME = C:\Program Files\Java\jdk-21.0.x
Path += %JAVA_HOME%\bin
```
...
c’est que Java/JAVA_HOME ne sont pas correctement configurés.

### 6️⃣ Étape Transform & Load (PySpark)

Une fois Java opérationnel :
...
### 7️⃣ Lancer Streamlit

Une fois les données transformées disponibles :

```bash
uv run streamlit run webapp/app.py
```

L’application lit les données dans `data/filtered/` et propose la visualisation interactive du footfall.

### 8️⃣ Airflow (optionnel / orchestration complète)

Pour orchestrer extraction + transformation en production :

- Démarrer Airflow (par exemple en local) :
...
- Activer les DAGs :
  ...
  - `banking_pipeline.py` : pipeline horaire temps réel.
  - `banking_pipeline_backfill.py` : backfill après panne.

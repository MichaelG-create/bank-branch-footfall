...
A Streamlit app (`webapp/app.py`) reads the transformed Parquet data via DuckDB and provides an interactive dashboard for exploring branch traffic patterns.

#### Key Features

**Time Series Visualization with Dual Charts:**
- Main panel displays daily visitor counts with toggleable overlays (4-day rolling average, relative thresholds).
- Secondary sparkline panel below shows percentage variation vs. the 4-day average for quick pattern recognition.
- Both panels share the x-axis, so zooming/hovering syncs across views.

**Threshold-Based Alerts:**
- Define an acceptable deviation range (e.g., ±20%) from the 4-day rolling average.
- Upper and lower threshold lines visualize the alert band.
- Yellow diamond markers highlight days when traffic breaks outside the thresholds.
- A dedicated alerts panel lists exceedances with footfall ratios.

**Flexible Time Filtering:**
- Select by year, then refine with months, weeks, or a custom date range.
- Filters are mutually exclusive: selecting months clears weeks, and vice versa.

**Single Agency vs. Multi-Agency Comparison:**
- **Single agency mode:** Choose a branch and optionally a specific counter (sensor). View detailed traffic for one location.
- **Multi-agency mode:** Compare traffic across multiple branches side-by-side with grouped variation bars.

**Data Toggle:**
- Switch between interactive graph and raw tabular data views with a single click.

[Public live application](https://bank-branch-footfall.streamlit.app/)

#### Single Agency View

![streamlit-single-agency](images/webapp-single-agency-screenshot.png)

The single agency view shows daily visitor count (blue line), 4-day rolling average (dashed gold line), and optional threshold bands. The variation sparkline below indicates percentage change, with green bars for increases and red for decreases.

#### Multi-Agency Comparison

![streamlit-multiple-agencies](images/webapp-multiple-agencies-screenshot.png)

The multi-agency comparison displays traffic trends across branches in different colors, with grouped variation bars below for easy visual comparison of each agency's daily performance.
...
- **Frontend:** Streamlit, Plotly (with subplots for time series + sparklines).
...
The application reads data from `data/filtered/` and provides an interactive visualization of bank branch footfall with:

- Dual time series + sparkline layout.
- Configurable threshold-based alerts.
- Flexible time range selection (years, months, weeks, or custom dates).
- Single agency and multi-agency comparison modes.
- Graph and data table views.
...
- Display helpers and alerting logic.

Run them from the project root, for example:

```bash
uv run pytest tests/ --cov=bank_footfall --cov-report=html
```
...
- Adding predictive models on top of the historical time series (e.g., ARIMA, Prophet).
- Implementing real-time alert notifications (email, Slack) when thresholds are breached.
...
- Deploying the full stack to a cloud provider (e.g., GCP) for scalability and resilience.

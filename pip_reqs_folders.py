                ...
                if has_python_files:
                    if subdir == "airflow/dags":  # Cas particulier pour airflow/dags
                        requirements_file = os.path.join(
                            base_path, "airflow", "requirements-airflow.txt"
                        )
                        subprocess.run(
                            [
                                "pipreqs",
                        ...
                        requirements_file = os.path.join(
                            root, f"requirements-{folder_name}.txt"
                        )
                        subprocess.run(
                            [
                                "pipreqs",
...
project_path = os.path.expanduser(
    "~/ProjetPerso/bank-branch-footfall"
)  # Remplace par ton chemin complet si nécessaire
